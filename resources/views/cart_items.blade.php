    ...
    <body>
        @isset($carts)
            ...
            @foreach($carts as $cart)
                <a href="/api/cart-item/cart/{{$cart->id}}">{{ $cart->id }}: {{ $cart->status }}: {{$cart->created_at}}</a>
                </br>
            @endforeach
        @endisset
        @isset($cartItems)
            <h1>Cart items</h1>
            <h3><i>Cart: {{$currentCart->id}}: {{$currentCart->status}}</i></h3>
            @foreach($cartItems as $item)
                <a href="/api/cart-item/{{$item->id}}">{{ $item->product->name }}</a>
                </br>
            @endforeach
        @endisset
        <hr/>
        @isset($products)
            ...
            @foreach($products as $product)
                <a href="/api/cart-item/cart/{{$currentCart->id}}/product/{{$product->id}}">{{$product->name}}</a>
                </br>
            @endforeach
            @endisset
        @endisset
        ...
        @isset($singleCartItem)
            ...
            <div class="alert alert-dark">
                ...
                <form method="POST" action="/api/cart-item/{{$singleCartItem->id}}/delete">
                    @csrf
                    <input type="submit" class="btn btn-danger" value="Delete"/>
                </form>

                <form method="POST" action="/api/cart-item/{{$singleCartItem->id}}/promotions/delete">
                    @csrf
