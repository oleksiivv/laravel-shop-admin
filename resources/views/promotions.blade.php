        ...
        @isset($cartItems)
            ...
            @foreach($cartItems as $cartItem)
                <div class="alert alert-warning">
                    <h1>Cart: <a href="/api/cart/{{$cartItem->cart_id}}">{{$cartItem->cart_id}}: {{$cartItem->cart->status}}</a></h1>
                    <h3>Item: <a href="/api/cart-item/{{$cartItem->id}}">{{$cartItem->product->name}}</a></h3>
                    <h4>Price: ${{$cartItem->price}}</h4>
                    <a href="/api/promotion/cart-item/{{$cartItem->id}}">Add promotion</a>
                </div>
                </br>
            @endforeach
        @endisset
        @isset($singlePromotion)
            <h1>Promotion with coupon {{$singlePromotion->coupon}}</h1>
