    ...
    <body>
        @isset($carts)
            @foreach($carts as $cart)
                <a href="/api/cart/{{$cart->id}}">{{ $cart->id }}: {{ $cart->status }}: {{$cart->created_at}}</a>
                </br>
            @endforeach
        @endisset
        <hr/>
        @isset($singleCart)
            <h1>Cart {{$singleCart->id}}</h1>
            <h4>{{ $singleCart->status }}</h4>
            <h4>{{ $singleCart->created_at }}</h4>
            <h4>Customer: {{$singleCart->customer->email}}</h4>
            <h4>Seller: {{$singleCart->worker}}</h4>
            <h3>Cart items</h3>
            @foreach($singleCart->cartItems as $item)
                <p><strong>Product: {{$item->product->name}}</strong><br/>${{$item->price}}</p>
                <h6>Promotions</h6>
                <ol>
